...
2. **Boundaries can be designed; traps must be learned** — and learned traps must have somewhere shared to go.
...
5. **The loop destroys its own audit trail.** Record design can only be installed at the start — decide up front what you will later want to prove.

The loop still runs every night. The audit is now part of it. The tools that made this audit possible — the attribution check, the digest, the shared home for learned constraints — are still specific to this one loop. As a next step, I'll try to generalize them so any loop can answer the same questions.
